...
boda = ["Me caso contigo 💍","Por supuesto que sí 💍❤️","No quiero 💍💔","Por supuesto que no 💍💔","Te amo Por supuesto que me caso contigo 💍"]
...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("funcionando")  
        await self.highrise.walk_to(Position(2.5 , 0.5 , 1.0 , "FrontRight"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrou na sala")  
        await self.highrise.send_whisper(user.id,f"{user.username} Bienvenid@ mi muchacho loco 👽👌🏻 Pásala bien con nosotros,si quieres beneficios habla con los moderadores y te darán información 😎")
...
Item(type='clothing', amount=1, id='bag-n_registrationavatars2023furrytail', account_bound=False, active_palette=-1),
    ...
    async def on_user_leave(self, user: User) -> None:
        ...
        await self.highrise.chat(f"Si necesita ayuda con mis comandos, utilice !help")